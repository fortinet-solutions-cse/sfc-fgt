...
floatIp_${VM_NAME}_fgt=${F_IP_1}

neutron port-list >port-list

p_A_fgt_${VM_NAME}_id=\$(cat port-list|grep p_A_fgt_$VM_NAME|awk '{print \$2}')
p_A_fgt_${VM_NAME}_ip=\$(cat port-list|grep p_A_fgt_$VM_NAME|awk '{print \$13}'|cut -d "\"" -f2)

p_B_fgt_${VM_NAME}_id=\$(cat port-list|grep p_B_fgt_$VM_NAME|awk '{print \$2}')
p_B_fgt_${VM_NAME}_ip=\$(cat port-list|grep p_B_fgt_$VM_NAME|awk '{print \$13}'|cut -d "\"" -f2)
...
openstack port delete p_B_fgt_$VM_NAME
openstack port delete p_A_fgt_$VM_NAME
...
openstack port create --network netM p_A_fgt_$VM_NAME
openstack port create --network netServerM p_B_fgt_$VM_NAME
. env_${VM_NAME}.sh
...
p_A_id=p_A_fgt_${VM_NAME}_id
p_B_id=p_B_fgt_${VM_NAME}_id
...
nova boot --flavor m1.fortigate --image "FortiGate_vwp_mac_disable" --nic net-name=mgmt --nic port-id=${!p_A_id} --nic port-id=${!p_B_id} --config-drive true --user-data myConfig_${VM_NAME}.txt --ephemeral size=5 fgt_${VM_NAME}
